...
def test_public_method_mapping_is_complete_canonical_and_version_matched() -> None:
    ...
    assert RULESET_VERSION == METHOD_RULESET_VERSION == "1.11.0"
    assert METHOD_VERSION == "1.5.0"
    assert METHOD_SPECIFICATION == "docs/method-v1.5.0.md"
    assert tuple(RULE_METHOD_REFERENCES) == rule_ids
    assert len(rule_ids) == len(RULE_METHOD_REFERENCES) == 67
...
def test_versioned_method_document_matches_packaged_metadata() -> None:
    ...
    assert len(index_rows) == len(rules) == 67
