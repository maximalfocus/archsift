...
def test_public_protocol_freezes_authoring_contract() -> None:
    ...
    assert "four cohorts have been run" in words
...
def test_committed_fourth_cohort_meets_the_frozen_criterion(
...
def test_preserved_third_cohort_remains_valid_historical_evidence(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = Path(__file__).parents[1]
    monkeypatch.chdir(root)
