...
def test_packaged_corpus_is_the_exact_governed_evolution() -> None:
    ...
    proposal_content = (
        files("archsift").joinpath("knowledge/architecture-v2.change.json").read_bytes()
    )
    base = load_snapshot(files("archsift").joinpath("knowledge/architecture-v1.json").read_bytes())
    ...
    assert proposal["public_issue"] == "https://github.com/maximalfocus/archsift/issues/101"
    summary = validate_graph_change(proposal, snapshot, base)
    assert summary["changed_entry_count"] == 2
    assert len(snapshot.nodes) == 114
    assert len(snapshot.relations) == 89
...
def test_graph_corpus_cli_human_json_quiet_and_no_writes(
    ...
    assert "114 nodes across 9 kinds; 89 relations across 7 kinds" in human
