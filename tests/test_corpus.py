...
def test_packaged_corpus_is_the_exact_governed_evolution() -> None:
    ...
    proposal_content = (
        files("archsift").joinpath("knowledge/architecture-v5.change.json").read_bytes()
    )
    base = load_snapshot(files("archsift").joinpath("knowledge/architecture-v4.json").read_bytes())
    ...
    assert proposal["public_issue"] == "https://github.com/maximalfocus/archsift/issues/133"
    summary = validate_graph_change(proposal, snapshot, base)
    assert summary["changed_entry_count"] == 2
    assert len(snapshot.nodes) == 117
    assert len(snapshot.relations) == 92
...
def test_graph_corpus_cli_human_json_quiet_and_no_writes(
    ...
    assert "117 nodes across 9 kinds; 92 relations across 7 kinds" in human
