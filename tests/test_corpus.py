...
def test_packaged_corpus_is_the_exact_governed_initial_publication() -> None:
    content = packaged_corpus_bytes()
    snapshot = packaged_corpus_snapshot()
    proposal_content = (
        files("archsift").joinpath("knowledge/architecture-v1.change.json").read_bytes()
    )
    ...
    assert proposal["change_kind"] == "initial-publication"
    assert proposal["base_snapshot"] is None
    assert proposal["behavior_change"] is False
    assert proposal["public_issue"] == "https://github.com/maximalfocus/archsift/issues/99"
    summary = validate_graph_change(proposal, snapshot)
...
def test_graph_corpus_cli_human_json_quiet_and_no_writes(
    ...
    assert "113 nodes across 9 kinds; 88 relations across 7 kinds" in human
