...
> [!IMPORTANT]
> ArchSift is currently pre-alpha. The CLI can validate typed evidence, task, problem-value, agency-necessity, autonomy-permission, and candidate-comparison facts, compose their prerequisite readiness, and inspect the packaged ruleset. The domain core can deterministically eliminate represented candidates, resolve evidence-complete architecture verdicts, canonicalise schema-version-1 dossiers and evidence-ledger entries with stable content identities, and compose canonical in-memory machine decision records. The domain filesystem boundary can also hash explicitly referenced workspace or caller-authorised external evidence artefacts without treating provenance as a path. The `assess` command, final persistent artefact-aware record identities, and Markdown decision reports are not implemented yet.
