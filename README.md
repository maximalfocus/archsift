...
> [!IMPORTANT]
> ArchSift is currently pre-alpha. The CLI can create and validate a versioned case workspace with a typed evidence ledger; task analysis and the assessment engine are not implemented yet.
...
`init` creates `case.yaml`, workspace guidance, and empty `evidence/` and `output/` directories. The dossier ledger distinguishes observations, assumptions, estimates, and known gaps without opening provenance metadata. `validate` safely checks the versioned dossier and fails closed on malformed, unsupported, unknown, duplicate, or unsafe input. See the [stable exit-code contract](docs/exit-codes.md).
