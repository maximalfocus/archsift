...
`report` renders a generated record as a standalone detailed HTML report beside it, at `output/sha256-<record-id>.detailed.html`. The report states the same content as the Markdown review view and is an output of the record rather than a separate authoritative artifact: its name restates the record's content identity and it derives no identity of its own. The document is self-contained and offline — no network resource, script, font, image, or external stylesheet — every authored string is rendered as escaped text rather than markup, attribute, or URL, the record's masking policy is applied to it, and identical inputs produce byte-identical HTML with the same reuse and never-overwrite discipline as the record itself.
