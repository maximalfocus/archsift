"""The immutable, evidence-backed initial architecture knowledge corpus."""
...
CORPUS_SNAPSHOT_RESOURCE = "knowledge/architecture-v1.json"
CORPUS_PROPOSAL_RESOURCE = "knowledge/architecture-v1.change.json"
...
def _validated_corpus() -> tuple[bytes, Snapshot]:
    ...
    validate_graph_change(proposal, snapshot)
    return content, snapshot


def packaged_corpus_bytes() -> bytes:
    """Return the exact canonical bytes of the initial public corpus."""
    return _validated_corpus()[0]


def packaged_corpus_snapshot() -> Snapshot:
    """Return the validated immutable initial public corpus snapshot."""
