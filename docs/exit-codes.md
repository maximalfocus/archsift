...
| `11` | `UNSUPPORTED_SCHEMA` | A dossier, decision record, graph snapshot, usability result, or method-review result declares a version this ArchSift version does not support. |
| `12` | `VALIDATION_FAILED` | A workspace, dossier, declared case language, graph snapshot, usability cohort, or method-review result violates its supported contract or success criterion. |
...
| `14` | `ARTEFACT_UNAVAILABLE` | Assessment cannot read an explicitly referenced artefact, the caller did not grant its external evidence root, or a requested report, comparison, graph snapshot, usability, or method-review record is unavailable. |
