...
| `11` | `UNSUPPORTED_SCHEMA` | A dossier, decision record, or usability result declares a version this ArchSift version does not support. |
| `12` | `VALIDATION_FAILED` | A workspace, dossier, or usability cohort violates its supported contract or success criterion. |
...
| `14` | `ARTEFACT_UNAVAILABLE` | Assessment cannot read an explicitly referenced artefact, the caller did not grant its external evidence root, or a requested comparison/usability record is unavailable. |
...
A diagnostic identifies its file, field, governing requirement, and remediation. Validation errors retain their existing codes; a valid but incomplete dossier may still be assessed successfully as `insufficient-evidence`. Use `assess --json` for exact canonical decision-record bytes, `compare --json` for exact canonical comparison bytes, and `usability-results --json` for a structured offline cohort result. Every command emits diagnostic JSON on failure with `--json`, or no output with `--quiet` when only the exit code is needed.
