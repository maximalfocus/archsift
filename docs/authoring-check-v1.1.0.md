# Simulated assisted-authoring check — protocol 1.1.0

Protocol 1.1.0 supersedes protocol 1.0.1 after four cohorts. It preserves the material set, the six
milestones, the transport and privacy contracts, and the success threshold; it changes the session
instructions in one way: the author inspects the published evidence-set profile (FR-021) of the bound
schema version and authors the dossier by filling its slots, so every session prepares the same
evidence set instead of transcribing the shape of the material. No cohort has been run under protocol
1.1.0, and no claim is made under it. Protocol 1.0.1 remains frozen at
[authoring-check-v1.0.1.md](authoring-check-v1.0.1.md) with its four committed cohort results, and
protocol 1.0.0 at [authoring-check-v1.md](authoring-check-v1.md).
...
5. Inspect the complete schema-version-3 contract with
   `archsift dossier-schema --schema-version 3 --json`, and the evidence-set profile of that
   version with `archsift dossier-schema --schema-version 3 --evidence-set --json`: the ordered
   slots, grouped by decision question, that every case is prepared into.
6. Author `synthetic-routing/case.yaml` by filling the profile's slots from the frozen material, in
   profile order. Leave a slot the material does not support explicitly missing or unknown rather
   than inventing an answer. Cite registrations only where the schema permits them and preserve the
   evidence authorship and attestation boundary.
...
- `inspect_schema`: schema version 3 and its evidence-set profile are emitted successfully before
  dossier authoring completes;
