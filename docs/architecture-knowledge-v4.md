# ArchSift architecture knowledge, declared baseline-retention evolution

This is the third governed evolution of ArchSift's finite, open-world reusable
architecture knowledge under FR-015. Its canonical graph schema v1 snapshot is
wheel-packaged at `archsift/knowledge/architecture-v4.json`; its canonical
evolution proposal is packaged beside it as `architecture-v4.change.json`,
names `architecture-v3.json` as its exact immutable base, and is governed by
public issue #129.
...
- immutable graph version:
  `gv1:6b07fdbdc861049ea35c23c4e40f145e6b3cde1f0df1b37ec655d5e2bc5eb063`
- snapshot content identity:
  `sha256:f87a3b9366d9a786f4e16ab6b57e88d4b06b2e6ee38cbb8d014d509741c23aa7`
- inventory: 116 nodes across all 9 node kinds and 91 relations across all 7
  relation kinds
...
Every rule in packaged ruleset 1.12.0 has one `decision-rule` node with the exact
rule ID and exact source-ID mapping from method 1.6.0. Each rule is reached by
...
archsift graph-corpus --json > architecture-v4.json
archsift graph-snapshot architecture-v4.json
archsift graph-view architecture-v4.json examples/graph-corpus-request.json --json
```
...
```bash
archsift graph-change architecture-v4.change.json architecture-v4.json --base-snapshot architecture-v3.json
```

This evolution adds the baseline-retention contradiction rule and its
source-mapped rationale relation, accompanying the dossier schema version 4
declaration that resolves a non-discriminating binding set as an authored
decision. Its proposal names the synthetic declared-retention counterexamples
and the regression tests that separate a satisfied prerequisite from a
contradiction.
