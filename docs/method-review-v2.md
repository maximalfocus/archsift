...
Each result also freezes its exact method, ruleset, and example-corpus binding. The packaged
binding registry is the sole compatibility authority; schema version fields constrain shape only,
and no version-ordering heuristic treats an unknown version as historical. Once a registered
binding is superseded, a valid result remains loadable and reports the covered versions with a
`criterion-met-superseded` or `criterion-not-met-superseded` JSON status and exit `16`. Quiet mode
