...
As of 2026-08-16, four cohorts have been run. The current
[result](../authoring-results.json), bound to source commit
`358d16596b71d5451a0cb7d85ad33a84b2202295`, validates with `criterion-met`: three of four sessions
passed. Byte-preserved historical [third](../authoring-results-3-criterion-not-met.json),
[second](../authoring-results-2-criterion-not-met.json), and
[first](../authoring-results-1-criterion-not-met.json) results remain `criterion-not-met`, with
zero, one, and one passing sessions respectively. The evidence is simulated and supports only the
agent-product assisted-authoring criterion; no human-author or real-decision-quality claim is made.
